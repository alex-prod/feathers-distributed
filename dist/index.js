...
var _cote = _interopRequireDefault(require("./cote"));
...
const debug = (0, _debug.default)('feathers-distributed');
...
function init(options) {
  return function () {
    ...
    let app = this; // Because options are forwarded and assigned to defaults options of services allocate an empty object if nothing is provided

    app.coteOptions = distributionOptions.cote || {};
    app.cote = distributionOptions.cote ? (0, _cote.default)(distributionOptions.cote) : (0, _cote.default)(); // We need to uniquely identify the app to avoid infinite loop by registering our own services

    app.uuid = (0, _v.default)();
    debug('Initializing feathers-distributed'); // This publisher publishes an event each time a local app service is registered
    ...
    app.servicePublisher = new app.cote.Publisher({
      ...
      log: false
    }, app.coteOptions)); // Also each time a new node pops up so that it does not depend of the initialization order of the apps
    ...
    app.servicePublisher.on('cote:added', data => {
      ...
      setTimeout(_ => {
        Object.getOwnPropertyNames(app.services).forEach(path => {
          app.servicePublisher.publish('service', {
            uuid: app.uuid,
            path
          });
    ...
    app.serviceSubscriber = new app.cote.Subscriber({
      ...
      log: false
    }, app.coteOptions)); // When a remote service is declared create the local proxy interface to it
    ...
    app.serviceSubscriber.on('service', serviceDescriptor => {
      ...
      debug('Registered remote service on path ' + serviceDescriptor.path); // registering hook object on every remote service

      if (distributionOptions.hooks) {
        app.service(serviceDescriptor.path).hooks(distributionOptions.hooks);
      }

      debug('Registered hooks on remote service on path ' + serviceDescriptor.path); // dispatch an event internally through node so that async processes can run

      app.emit('service', serviceDescriptor); // register error handler
    ...
    app.use = function (path, service) {
      ...
      superUse.apply(app, arguments); // Note: middlewares are not supported
      ...
      if (typeof service === 'object' && !service.remote) {
        ...
        debug('Published local service on path ' + path); // Register the responder to handle remote calls to the service

        service.responder = new _service.LocalService({
          app,
          path: (0, _commons.stripSlashes)(path)
        });
      }
