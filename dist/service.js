...
const debug = (0, _debug.default)('feathers-distributed:service'); // This is the Feathers service abstraction for a cote requester on remote
...
class RemoteService {
  ...
  setup(app, path) {
    ...
    this.path = path;
    debug('Requester created for remote service on path ' + this.path); // Create the subscriber to listen to events from other nodes
...
class LocalService extends _cote.default.Responder {
  constructor(options) {
    ...
    let service = app.service(path); // Answer requests from other nodes
